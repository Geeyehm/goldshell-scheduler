...
(or {"time": ..., "level": N} instead of "mode" for direct control).

The active entry is the most recent one (walking back day by day, respecting
optional weekday restrictions) whose time is at or before `now` - i.e. standard
"last transition wins" schedule semantics, so a schedule set once continues to
apply correctly across midnight/day boundaries without needing an entry at 00:00.
"""
